...
class CustomMountainCarEnv_acceleration(gym.Wrapper):       # Reward wird vergeben wenn car sich in bewegungsrichtung beschleunigt
    ...
    def step(self, action):
        ...
        if oldVelocity > 0 and action == 2: reward = 3 # moving right and pushing right = a reward
        if oldVelocity < 0 and action == 0: reward = 3 # moving left and pushing left = a reward
